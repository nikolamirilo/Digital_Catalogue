...
export async function duplicateMenu(id: string) {
  ...
  let count = 1;
  while (true) {
    const { data: exists } = await supabase.from('restaurants').select('id').eq('name', tryName);
    if (!exists || exists.length === 0) break;
    tryName = `${newName}${suffix}${count}`;
    count++;
  }
  // Insert the duplicate
  const { data: newData, error: insertError } = await supabase.from('restaurants').insert({ ...rest, name: tryName }).select().single();
