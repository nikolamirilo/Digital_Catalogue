  ...
  return (
     <Card className="space-y-8 p-6 sm:p-8 bg-white/95 border border-product-border shadow-product-shadow rounded-2xl" type="form">
      ...
      {formData.services.map((category, categoryIndex) => (
        <Card key={categoryIndex} className="space-y-6 p-6 sm:p-8 bg-product-background/50 border border-product-border shadow-product-shadow rounded-xl" type="form">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-xl font-bold text-product-foreground" style={{ fontFamily: 'var(--font-playfair-display), var(--font-inter), serif' }}>Category {categoryIndex + 1}</h3>
            <Button
              type="button"
              variant="destructive"
              size="sm"
          ...
          <div className="space-y-3">
            <Label htmlFor={`category-name-${categoryIndex}`} className="text-product-foreground font-medium" style={{ fontFamily: 'var(--font-inter), sans-serif' }}>Category Name</Label>
            <Input
          ...
          <div className="space-y-4">
            <Label htmlFor={`category-layout-${categoryIndex}`} className="text-product-foreground font-medium text-lg" style={{ fontFamily: 'var(--font-inter), sans-serif' }}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {layouts.map((layoutOption) => (
                ...
                >
                  ...
                  />
                  <p className="text-center text-sm mt-2 font-medium text-product-foreground" style={{ fontFamily: 'var(--font-inter), sans-serif' }}>{layoutOption.label}</p>
                </div>
              ))}
            </div>
          </div>
